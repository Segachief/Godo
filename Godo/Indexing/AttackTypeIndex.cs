...
{
    public class AttackTypeIndex
    {
        public static int[][][] GetAttackData(int[][] jaggedSceneInfo, string targetScene, Random rnd)
        {
            int r = 0;
            int o = 0;
            int c = 0;
                                ...
                                while ((bytesRead = zipInput.Read(uncompressedScene, 0, 7808)) != 0)
                                {
                                    ...
                                    while (enemyCount < 3) // Iterates through the 3 registerable enemy slots in this scene
                                    {
                                        ...
                                        if (uncompressedScene[c + 1] != 255)
                                        {
                                            ...
                                            // Our arrays have a bunch of 0 values that we need to get rid of (if possible).
                                            // These loops dupe valid values to 'empty' entries to improve anim assignment accuracy later
                                            int dupeAssignment = 0;
                                            int terminate = 0;
                                            while (physCount < 16)
                                            {
                                                while (dupeAssignment == 0 && terminate < 32)
                                                {
                                                    dupeAssignment = rnd.Next(0, physAnims.Length);
                                                    terminate++;
                                                }
                                                physAnims[physCount] = physAnims[dupeAssignment];
                                                physCount++;
                                                dupeAssignment = 0;
                                            }
                                            ...
                                            while (magCount < 16)
                                            {
                                                while (dupeAssignment == 0 && terminate < 32)
                                                {
                                                    dupeAssignment = rnd.Next(0, magAnims.Length);
                                                    terminate++;
                                                }
                                                magAnims[magCount] = magAnims[dupeAssignment];
                                                magCount++;
                                                dupeAssignment = 0;
                                            }
                                            ...
                                            while (miscCount < 16)
                                            {
                                                while (dupeAssignment == 0 && terminate < 32)
                                                {
                                                    dupeAssignment = rnd.Next(0, miscAnims.Length);
                                                    terminate++;
                                                }
                                                miscAnims[miscCount] = miscAnims[dupeAssignment];
                                                miscCount++;
                                                dupeAssignment = 0;
                                            }
